...
class BaseBlock(nn.Module):
    ...
    def __init__(self, input_channel, output_channel, kernel_size, t = 6, downsample = False):
        """
        ...
        # attention
        self.attention = CoordAtt(c,c,num_frequency=self.num_frequency,kernel_size=kernel_size,reduction_coord=self.reduction_coord, reduction_dct=self.reduction_dct)
        

    def forward(self, inputs):
        # main path
        x = F.relu6(self.bn1(self.conv1(inputs)), inplace = True)
        x = F.relu6(self.bn2(self.attention(self.conv2(x))), inplace = True)
