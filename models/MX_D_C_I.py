...
class I2RBlock(nn.Module):
    def __init__(self, inp, oup, num_frequency, f, stride, expand_ratio, keep_3x3=False):
        ...
        if expand_ratio == 2:
            self.conv = nn.Sequential(
                ...
                CoordAtt(inp,inp,num_frequency=num_frequency,kernel_size=self.feature_size),
                nn.BatchNorm2d(inp),
                nn.ReLU6(inplace=True),
                # pw-linear
                ...
                # pw-linear
                nn.Conv2d(hidden_dim, oup, 1, 1, 0, bias=False),
                #CoordAtt(oup,oup,num_frequency=num_frequency,kernel_size=self.feature_size),
                nn.BatchNorm2d(oup),
                nn.ReLU6(inplace=True),
                # dw
            ...
            self.conv = nn.Sequential(
                ...
                # pw-linear
                nn.Conv2d(hidden_dim, oup, 1, 1, 0, bias=False),
                #CoordAtt(oup, oup, num_frequency=8, kernel_size=self.feature_size),
                nn.BatchNorm2d(oup),
                nn.ReLU6(inplace=True),
                # dw
                nn.Conv2d(oup, oup, 3, stride, 1, groups=oup, bias=False),
...
class I2RNet(nn.Module):
    def __init__(self, num_classes=1000, width_mult=1.,num_frequency=4):
        ...
        for t, c, n, s in self.cfgs:
            ...
            if c == 1280 and width_mult < 1:
                output_channel = 1280
            layers.append(block(input_channel, output_channel, num_frequency, f, s, t, n==1 and s==1))
            if s==2:
                f=int(f/2)
            input_channel = output_channel
            for i in range(n-1):
                layers.append(block(input_channel, output_channel, num_frequency, f, 1, t))
                input_channel = output_channel
